...
CORRECTION_SYSTEM_PROMPT = """صحح الأخطاء الإملائية في هذه المقاطع. حافظ على اللهجة كما هي. لا تدمج أو تقسم المقاطع. أرجع JSON فقط بهذا الشكل:
...
async def transcribe(audio, dialect="auto", initial_prompt=None, apply_llm_correction=None):
    ...
    Whisper params reverted to faster-whisper defaults.
    Only non-default param: condition_on_previous_text=False (kills chorus loops).
    All spelling/punctuation/dialect cleanup happens in LLM post-correction.
    """
    if _model is None:
    ...
    segments, info = _model.transcribe(
        ...
        condition_on_previous_text=False,   # ONLY non-default — kills chorus loops
